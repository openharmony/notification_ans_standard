...
{
    ANS_LOGI("SetSubscribeCallbackData start");
    if (request == nullptr) {
...
napi_value On(napi_env env, napi_callback_info info)
{
    ...
    AsyncCallbackInfoOn *asynccallbackinfo = new (std::nothrow) AsyncCallbackInfoOn{
        .env = env, .asyncWork = nullptr, .objectInfo = objectInfo, .type = type, .callback = callback};

    if (!AddAsyncCallbackInfo(objectInfo, asynccallbackinfo)) {
        return Common::NapiGetNull(env);
    }
    ...
    const napi_env &env, const size_t &argc, const napi_value (&argv)[OFF_MAX_PARA], std::string &type)
{
    ...
    if (!(type == CONSUME || type == CANCEL || type == UPDATE || type == SUBSCRIBE || type == UNSUBSCRIBE ||
            type == DIE || type == DISTURB_MODE_CHANGE)) {
        ANS_LOGE("type parameter is error");
        return nullptr;
    }
...
napi_value Off(napi_env env, napi_callback_info info)
{
    ...
    AsyncCallbackInfoOff *asynccallbackinfo = new (std::nothrow) AsyncCallbackInfoOff{
    ...
    napi_create_async_work(env,
        ...
        [](napi_env env, napi_status status, void *data) {
            ...
            DelAsyncCallbackInfo(asynccallbackinfo->objectInfo, asynccallbackinfo->type);
...
napi_value SubscriberConstructor(napi_env env, napi_callback_info info)
{
    ...
    napi_wrap(env,
        ...
        [](napi_env env, void *data, void *hint) {
            ...
            DelAsyncCallbackInfo(objectInfo);
...
bool CreateSubscriberInstanceRecord(SubscriberInstance **subscriber)
{
    ...
    *subscriber = new (std::nothrow) SubscriberInstance();
...
{
    ANS_LOGI("AddAsyncCallbackInfo start");
