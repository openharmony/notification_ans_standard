...
ErrCode AnsManagerProxy::RemoveSlots(const std::vector<std::string> &slotIds)
{
    if (slotIds.empty()) {
        ANS_LOGW("[RemoveSlots] fail: slotIds is empty.");
        return ERR_ANS_INVALID_PARAM;
    }

    MessageParcel data;
    if (!data.WriteInterfaceToken(AnsManagerProxy::GetDescriptor())) {
        ANS_LOGW("[RemoveSlots] fail: write interface token failed.");
        return ERR_ANS_PARCELABLE_FAILED;
    }

    if (!data.WriteStringVector(slotIds)) {
        ANS_LOGW("[RemoveSlots] fail:: write slotIds failed.");
        return ERR_ANS_PARCELABLE_FAILED;
    }

    MessageParcel reply;
    MessageOption option = {MessageOption::TF_SYNC};
    ErrCode result = InnerTransact(REMOVE_SLOTS, option, data, reply);
    if (result != ERR_OK) {
        ANS_LOGW("[RemoveSlots] fail: transact ErrCode=%{public}d", result);
        return ERR_ANS_TRANSACT_FAILED;
    }

    if (!reply.ReadInt32(result)) {
        ANS_LOGW("[RemoveSlots] fail: read result failed.");
        return ERR_ANS_PARCELABLE_FAILED;
    }
...
ErrCode AnsManagerProxy::GetSlot(const std::string &slotId, sptr<NotificationSlot> &slot)
{
    if (slotId.empty()) {
        ANS_LOGW("[GetSlot] fail: slotId is null.");
        return ERR_ANS_INVALID_PARAM;
    }

    MessageParcel data;
    if (!data.WriteInterfaceToken(AnsManagerProxy::GetDescriptor())) {
        ANS_LOGW("[GetSlot] fail: write interface token failed.");
        return ERR_ANS_PARCELABLE_FAILED;
    }

    if (!data.WriteString(slotId)) {
        ANS_LOGW("[GetSlot] fail:: write slotId failed");
        return ERR_ANS_PARCELABLE_FAILED;
    }

    MessageParcel reply;
    MessageOption option = {MessageOption::TF_SYNC};
    ErrCode result = InnerTransact(GET_SLOT, option, data, reply);
    if (result != ERR_OK) {
        ANS_LOGW("[GetSlot] fail: transact ErrCode=%{public}d", result);
        return ERR_ANS_TRANSACT_FAILED;
    }

    if (!reply.ReadInt32(result)) {
        ANS_LOGW("[GetSlot] fail: read result failed.");
        return ERR_ANS_PARCELABLE_FAILED;
    }

    slot = reply.ReadParcelable<NotificationSlot>();
    if (slot == nullptr) {
        ANS_LOGW("[GetSlot] fail: read slot failed");
        return ERR_ANS_PARCELABLE_FAILED;
    }
