...
const std::map<uint32_t, std::function<ErrCode(AnsManagerStub *, MessageParcel &, MessageParcel &)>>
    AnsManagerStub::interfaces_ = {
        ...
        {AnsManagerStub::REMOVE_SLOTS,
            std::bind(&AnsManagerStub::HandleRemoveSlots, std::placeholders::_1, std::placeholders::_2,
                std::placeholders::_3)},
        ...
        {AnsManagerStub::ADD_SLOT_GROUPS,
            std::bind(&AnsManagerStub::HandleAddSlotGroups, std::placeholders::_1, std::placeholders::_2,
                std::placeholders::_3)},
        {AnsManagerStub::GET_SLOT,
            std::bind(
                &AnsManagerStub::HandleGetSlot, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3)},
...
ErrCode AnsManagerStub::HandleRemoveSlots(MessageParcel &data, MessageParcel &reply)
{
    std::vector<std::string> slotIds;
    if (!data.ReadStringVector(&slotIds)) {
        ANS_LOGW("[HandleRemoveSlots] fail: read slotIds failed");
        return ERR_ANS_PARCELABLE_FAILED;
    }

    ErrCode result = RemoveSlots(slotIds);

    if (!reply.WriteInt32(result)) {
        ANS_LOGW("[HandleRemoveSlots] fail: write result failed, ErrCode=%{public}d", result);
        return ERR_ANS_PARCELABLE_FAILED;
    }
    return ERR_OK;
}
...
ErrCode AnsManagerStub::HandleGetSlot(MessageParcel &data, MessageParcel &reply)
{
    std::string slotId;
    if (!data.ReadString(slotId)) {
        ANS_LOGW("[HandleGetSlot] fail: read slotId failed");
        return ERR_ANS_PARCELABLE_FAILED;
    }

    sptr<NotificationSlot> slot;
    ErrCode result = GetSlot(slotId, slot);

    if (!reply.WriteInt32(result)) {
        ANS_LOGW("[HandleGetSlot] fail: write result failed, ErrCode=%{public}d", result);
        return ERR_ANS_PARCELABLE_FAILED;
    }

    if (!reply.WriteParcelable(slot)) {
        ANS_LOGW("[HandleGetSlot] fail: write slot failed.");
        return ERR_ANS_PARCELABLE_FAILED;
    }
    return ERR_OK;
}
...
ErrCode AnsManagerStub::RemoveSlots(const std::vector<std::string> &slotIds)
{
    ANS_LOGW("AnsManagerStub::RemoveSlots called!");
    return ERR_OK;
}
...
ErrCode AnsManagerStub::GetSlot(const std::string &slotId, sptr<NotificationSlot> &slot)
{
    ANS_LOGW("AnsManagerStub::GetSlot called!");
    return ERR_OK;
}
